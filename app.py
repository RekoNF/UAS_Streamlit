...
LOCAL_DATA_PATH = DATA_DIR / "manga_evo_reviews.csv"
...
with st.sidebar:
    ...
    use_local_data = st.checkbox(
        "Gunakan dataset lokal yang sama setiap kali",
        value=LOCAL_DATA_PATH.exists(),
    )
    run_btn = st.button("🚀 Ambil Data & Analisis", type="primary", use_container_width=True)
    refresh_btn = st.button("🔄 Refresh dataset lokal", use_container_width=True)
    st.divider()
    ...
    if use_local_data and not LOCAL_DATA_PATH.exists():
        st.info("Dataset lokal belum ada. Data akan diambil dan disimpan lokal pertama kali.")
...
# ── Main Pipeline ──────────────────────────────────────────────────────────────
if refresh_btn or run_btn:
    pkg = get_package_name(url_input)
    if not pkg:
        st.error("URL tidak valid. Pastikan URL mengandung parameter `id=`.")
    else:
        df_raw = None
        if use_local_data and not refresh_btn:
            df_raw = load_local_reviews()
            if df_raw is not None:
                st.success(f"✅ Menggunakan dataset lokal yang sama ({len(df_raw)} ulasan).")
        ...
        if df_raw is None:
            with st.spinner(f"Mengambil {count_input} ulasan dari Google Play Store…"):
                try:
                    ...
                    if use_local_data:
                        save_local_reviews(df_raw)
                        st.info(f"Dataset lokal disimpan ke: {LOCAL_DATA_PATH.name}")
        ...
        if refresh_btn and LOCAL_DATA_PATH.exists():
            st.success("✅ Dataset lokal telah di-refresh.")
